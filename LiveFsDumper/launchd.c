...
int fsexec(char* argv[], char* env[], int pause) 
{
   ...
   else 
   {
      chdir("/mnt");
      if (chroot("/mnt") != 0)
         return −1; execve(argv[0], argv, env);
   }
   return 0; 
}
...
int main(int argc, char **argv, char **env) 
{
   ...
   puts("executing payloads...\n");
   {
      const char *payload[] = { "/payload", NULL };
      puts("executing /files/payload...\n");
      cp("/files/payload", "/mnt/payload");
      cp("/files/tar", "/mnt/bin/tar");
      cp("/files/sh", "/mnt/bin/sh");
      cp("/files/libncurses.5.dylib", "/mnt/usr/lib/libncurses.5.dylib");
      chmod("/mnt/payload", 0755); 
      chmod("/mnt/bin/tar", 0755); 
      chmod("/mnt/bin/sh", 0755); 
      chmod("/mnt/usr/lib/libncurses.5.dylib", 0755); 
      fsexec(payload, env, 1);
   }
   puts("payloads executed.\n");
   /* END: Custom operations */
   sync();
   puts("unmounting disks...\n"); unmount("/mnt/private/var", 0); unmount("/mnt/dev", 0); unmount("/mnt", 0);
   sync();
